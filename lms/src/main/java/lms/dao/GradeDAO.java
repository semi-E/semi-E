...
public class GradeDAO {
		...
		public static ArrayList<HashMap<String, Object>> selectGradeList1(int studentNo, int classApplyNo) throws Exception{
			...
			String sql = "SELECT g.student_no AS studentNo,\n"
					...
					+ "       (a.CAL_COUNT * 20) AS attendanceScore,\n"
					+ "       (sa.ASS_COUNT * 20) AS assignmentScore,\n"
					+ "       TRUNCATE(((g.mid_exam * 0.3 / 30 * 100) + (g.final_exam * 0.3 / 30 * 100) + (a.CAL_COUNT * 20)) + (sa.ASS_COUNT * 20), 0) AS totalScore\n"
					+ "FROM grade g\n"
					+ "JOIN (\n"
					+ "        SELECT student_no,\n"
					+ "               class_apply_no,\n"
					...
					+ "                                 WHEN state = '/' THEN 0.5 \n"
					+ "                                 ELSE 0 \n"
					+ "                            END) / COUNT(*), 2) AS CAL_COUNT\n"
					+ "          FROM attendance \n"
					+ "         GROUP BY student_no, class_apply_no\n"
					+ "       ) a \n"
					+ "  ON g.student_no = a.student_no\n"
					+ " AND g.class_apply_no = a.class_apply_no\n"
					+ "JOIN (\n"
					+ "        SELECT student_no,\n"
					+ "               class_apply_no,\n"
					...
					+ "         GROUP BY student_no, class_apply_no\n"
					+ "       ) sa\n"
					+ "  ON g.student_no = sa.student_no\n"
					+ " AND g.class_apply_no = sa.class_apply_no\n"
					+ "WHERE g.student_no = 20240102 \n"
					+ "  AND g.class_apply_no = 3;  ";
